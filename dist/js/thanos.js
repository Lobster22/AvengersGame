$(function() {
  const fakeGameBoard = document.querySelector(".fake-game-board");
...
$(function() {
  $(".thanos").click(function() {
    html2canvas(gameBoard)
      .then(canvas => {
        fakeGameBoard.appendChild(canvas);
      })
      .then(() => {
        gameBoard.classList.add("byebye");
      });
  });
});
*/
  ...
  $(".thanosImg").click(function() {
    html2canvas($(gameBoard)[0]).then(canvas => {
  ...
  let snapThanos = new Image();
  snapThanos.src = "/img/thanos_snap.png";
});
